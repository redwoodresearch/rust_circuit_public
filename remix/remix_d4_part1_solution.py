...
"""
# %%
circuit = ex.wrap_in_var(
    treeified_circuit, ref_datum
...
# %%
if "SOLUTION":
    out2 = InterpNode(cond_sampler=FuncSampler(x0_or_x1_gt_3_agrees_with_label), name="out")  # type: ignore
    x0_prime2 = out2.make_descendant(FuncSampler(x0_val), name="x0'")  # type: ignore
    x1_prime2 = out2.make_descendant(FuncSampler(x1_val), name="x1'")  # type: ignore
    y_prime2 = out2.make_descendant(FuncSampler(label_val), name="y'")  # type: ignore
    ...
    out_batch2 = Experiment(loss, ds, corr2, num_examples=5000, random_seed=11).scrub().evaluate(eval_settings)
    print(f"Scrubbed: {out_batch2.mean()}")
    print(f"Original: {loss_with_data.evaluate().mean()}")
else:
    """TODO: Your code here"""
...
# %%
if "SOLUTION":
    out3a = InterpNode(cond_sampler=ExactSampler(), name="out")  # type: ignore
    ...
    out_batch3a = Experiment(loss, ds, corr3a, num_examples=5000, random_seed=11).scrub().evaluate(eval_settings)

    out3b = InterpNode(cond_sampler=ExactSampler(), name="out")  # type: ignore
    ...
    out_batch3b = Experiment(loss, ds, corr3b, num_examples=5000, random_seed=11).scrub().evaluate(eval_settings)
    print(f"x0 -> A randomly scrubbed: {out_batch3a.mean()}")
    print(f"x0 -> A not scrubbed: {out_batch3b.mean()}")
else:
    """TODO: Your code here"""
# %%
r"""
...
if "SOLUTION":
    ...
    out_batch4 = Experiment(loss, ds, corr4, num_examples=5000, random_seed=11).scrub().evaluate(eval_settings)
...
if "SOLUTION":
    ...
    out_batch5 = Experiment(loss, ds, corr5, num_examples=5000, random_seed=11).scrub().evaluate(eval_settings)
    print(f"Scrubbed: {out_batch5.mean()}")
    print(f"Original: {loss_with_data.evaluate().mean()}")
else:
    """TODO: Your code here"""
# %%
