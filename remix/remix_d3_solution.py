...
def sample_and_evaluate(c: rc.Circuit, num_samples: int = 16 * 128, batch_size=32) -> float:
    ...
    sampler = rc.Sampler(
        rc.RandomSampleSpec((num_samples,), seeder=seeder),
        run_on_sampled=run_on_sampled,
        device_dtype=c.device_dtype,
    )
...
if "SOLUTION":
    ...
    k_matcher = k_ind_input_matcher.chain(
        rc.restrict("idxed_embeds", term_early_at="b0.a.prev")
    )
...
# %%
if "SOLUTION":
    mask_prev = rc.Array(torch.diag(torch.ones((seq_len - 1,)), diagonal=-1).cuda(), name="mask_prev")
