...
if False:
    for node, matcher in tqdm(
        template_corr.corr.items()
    ):  # can be skipped / stopped if timing out
        assert (
...
def logit_diff_from_logits_dataset(
    dataset: Dataset, logits: torch.Tensor, mean=True,
):
...
#%% [markdown]
# Now make the correspondence that we'll actually use
# We use nodes with the same names as nodes in the template correspondence, but no parents and children (at the start)

#%%

# setup WANDB!
if USING_WANDB:
    wandb.init(project="acdc", name="acdc_test")
