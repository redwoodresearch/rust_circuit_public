...
class Add(Circuit):
    ...
    def __init__(self, *inputs: Circuit, name: Optional[str] = None):
        ...
        self.shape = tuple(t.broadcast_shapes(*[inp.shape for inp in inputs]))

    def evaluate(self) -> t.Tensor:
        "SOLUTION"
        if not self.inputs:
            return t.tensor(0.0)
        cur = t.zeros(self.shape)
...
"""
# %%
# TBD lowpri: arguably we should bite the bullet on GeneralFunction. We gain some insight into what's happening on Day 2 with last_dim_size.
# GeneralFunction requirements
# - UUID for serialization - early to present this when we don't intend to implement serialization ourselves.
# - For the GeneralFunction base to do output shape inference, needs a ShapeInfo structure from the derived class that discusses batching mask, num_non_batchable (starting from the right), and num_removed_from_end for reductions - I think we don't properly support reduction from any other dimensions right now? This seems subject to change in real circuits.
...
class Relu(Circuit):
    ...
    def __init__(self, input: Circuit, name: Optional[str] = None):
        "SOLUTION"
        if name is None:
            name = f"relu({input.name})"
        super().__init__(input, name=name)
        self.shape = input.shape

    def evaluate(self) -> t.Tensor:
        "SOLUTION"
        return torch.nn.functional.relu(self.inputs[0].evaluate())
...
def compute_spec_sizes(input_axes: list[Axes], node_shapes: list[tuple[int, ...]]) -> list[int]:
    ...
    "SOLUTION"
    sizes = [-1] * len(string.ascii_letters)
    for in_ax, node_shape in zip(input_axes, node_shapes):
        for i, size in zip(in_ax, node_shape):
            if sizes[i] in (-1, 0, 1):
                # first occurrence (-1), or 0 and 1 are compatible with anything
                sizes[i] = size
            else:
                # ensure compatible
                assert sizes[i] == size or size in (0, 1)
    return sizes
...
class Einsum(Circuit):
    ...
    def evaluate(self) -> t.Tensor:
        ...
        if not self.inputs:
            return t.tensor(1.0)
        einsum_args: list[Union[t.Tensor, Axes]] = []
        for inp, in_axes in zip(self.inputs, self.spec.input_axes):
            einsum_args.append(inp.evaluate())
            einsum_args.append(in_axes)
        einsum_args.append(self.spec.output_axes)
        out = t.einsum(*einsum_args)
        assert out.shape == self.shape, (self.shape, out.shape)
        return out
...
def scalar_mul(x: Circuit, number: float, name: Optional[str] = None) -> Einsum:
    ...
    """
    "SOLUTION"
    ax = string.ascii_letters[: len(x.shape)]
    return Einsum.from_einsum_string(f"{ax},->{ax}", x, Scalar(number, ()), name=name)


print("Testing scalar multiplication")

adder = Add(
    Array(t.tensor([1.0, 2.0]), name="first"),
    Array(t.tensor([3.0, 4.0]), name="second"),
    name="adder",
)
...
## TBD lowpri: they'll want to use Circuit.mul_scalar later, mention it and bind it
...
def minus(positive: Circuit, negative: Circuit, name: Optional[str] = None) -> Add:
    ...
    "SOLUTION"
    if name is None:
        name = f"{positive.name} - {negative.name}"
    return Add(positive, scalar_mul(negative, -1), name=name)
...
## TBD lowpri: mention Circuit.sub here
...
def _repr_tree(circuit: Circuit, depth: int, mutable_lines: list[str]) -> None:
    ...
    """
    "SOLUTION"
    if isinstance(circuit, Einsum):
        line = f"{circuit.__class__.__name__} {circuit.spec.to_einsum_string()} {circuit.shape}"
    else:
        line = f"{circuit.__class__.__name__} {circuit.shape}"
    mutable_lines.append(f'{" " * depth}' + f' "{circuit.name}" ' + line)
    for inp in circuit.inputs:
        _repr_tree(inp, depth + 1, mutable_lines)
...
def make_linear(weight: Array, bias: Array, input: Circuit, name: str) -> Add:
    ...
    "SOLUTION"
    return Add(Einsum.from_einsum_string("oi, i -> o", weight, input), bias, name=name)
...
def model_to_circuit(model: TwoLayerSkip, input: t.Tensor) -> Add:
    ...
    """
    "SOLUTION"
    input_arr = Array(input, name="x")
    first = make_linear(
        Array(model.first.weight, name="first_w"),
        Array(model.first.bias, name="first_b"),
        input_arr,
        name="first",
    )
    return make_linear(
        Array(model.last.weight, name="last_w"),
        Array(model.last.bias, name="last_b"),
        Add(
            Relu(first, name="relu"),
            input_arr,
            name="skip",
        ),
        name="last",
...
if "SKIP":
    # For debug purposes, build the model in Rust circuits as well
    def make_linear_rust(weight: rc.Array, bias: rc.Array, input: rc.Circuit, name: str) -> rc.Add:
        return rc.Add(rc.Einsum.from_einsum_string("oi, i -> o", weight, input), bias, name=name)

    def model_to_circuit_rust(model: TwoLayerSkip, input: t.Tensor) -> rc.Add:
        """Return a new Circuit which computes model(input) when called."""
        input_arr = rc.Array(input, name="x")
        first = make_linear_rust(
            rc.Array(model.first.weight, name="first_w"),
            rc.Array(model.first.bias, name="first_b"),
            input_arr,
            name="first",
        )
        return make_linear_rust(
            rc.Array(model.last.weight, name="last_w"),
            rc.Array(model.last.bias, name="last_b"),
            rc.Add(
                rc.relu(first, name="relu"),
                input_arr,
                name="skip",
            ),
            name="last",
    ...
    r_circuita = model_to_circuit_rust(model_a, inp)
...
# %%
if "SKIP":
    # CM: regex is annoying and complicated right now
    @attrs.define(frozen=True)
    class Regex:
        """Wraps a regular expression.

        One funky thing to be aware of is that normally a period in a regular expression pattern matches any character, but we historically use literal periods in names. For example, head 8 in layer 2 would be called "2.8".

        For this reason, this class munges the pattern by default so that period characters only match literal periods.

        Like rc.Regex, but rc.Regex uses Rust's regular expression engine for evaluation.
        TBD: go over relevant differences - I think we don't anchor to start so you may need ^ character?
        """
        ...
        def call(self, s: str) -> bool:
            # TBD: this is actually like a search with the munged period and Rust rules - confusing
    ...
    class RegexMatcher:
        ...
        def __call__(self, circuit: Circuit) -> bool:
            "SOLUTION"
            return self.regex.call(circuit.name)


# TBD lowpri: is this clearer to Python users if we have an ABC BaseMatcher?
...
class TypeMatcher:
    ...
    def __call__(self, circuit: Circuit) -> bool:
        "SOLUTION"
        return isinstance(circuit, self.cls)
...
class AnyMatcher:
    ...
    def __call__(self, circuit: Circuit) -> bool:
        "SOLUTION"
        return any(o(circuit) for o in self.options)
...
class AllMatcher:
    ...
    def __call__(self, circuit: Circuit) -> bool:
        "SOLUTION"
        return all(o(circuit) for o in self.options)
...
MatcherIn = Union[
    ...
    # TBD lowpri: collections.abc.Set supports generics now, any reason not to change this in real Circuits?
    # set[Circuit],
    # frozenset[Circuit],
    # Circuit, # TBD lowpri: don't think they need this today?
]
...
class Matcher:
    ...
    def __init__(self, *inps: MatcherIn):
        ...
        inner: MatcherData
        if len(inps) == 0:
            inner = BoolMatcher(False)
        elif len(inps) == 1:
            inp = inps[0]
            if isinstance(inp, bool):
                inner = BoolMatcher(inp)
            elif isinstance(inp, str):
                inner = NameMatcher(inp)
            # elif isinstance(inp, Regex):
            #     inner = RegexMatcher(inp)
            elif isinstance(inp, (set, frozenset)):
                inner = AnyMatcher([Matcher(cast(MatcherIn, sub)) for sub in inp])
            elif inspect.isclass(inp) and issubclass(inp, Circuit):
                inner = TypeMatcher(inp)
            elif callable(inp):
                # We don't know the signature of inp is really correct
                inner = CallableMatcher(cast(Callable[[Circuit], bool], inp))
            else:
                raise ValueError(inp)
        else:
            inner = AnyMatcher([Matcher(inp) for inp in inps])
    ...
    #     """Shortcut to create a Regex matcher."""
    #     return Matcher(Regex(pattern, escape_dot))

    # omitted: iterativematcher stuff, get_paths, a bunch of other helpers that seem unimportant. update has the BasicUpdate vs Updater ugliness
...
# TBD lowpri: test that short circuiting works

# print("Testing match by regex")
# rm = Matcher(Regex("x"))
# assert rm(x_node)
# actual = {a.name for a in rm.get(circuit_a)}
# assert actual == {"x"}
...
# TBD: find a place to introduce evaluate_many.
# %%
"""
### Beyond Matcher
...
# Note: rc.TransformIn also includes the Transform class, which is just a serializable representation of the Callable - we can safely ignore it here
...
class BasicUpdater:
    ...
    def update(self, root: Circuit, matcher: MatcherIn) -> Circuit:
        ...
        """
        # TBD: for debugging i would love to have this also log (to an instance variable?) all nodes that were matched & transformed
    ...
    def _update_impl(self, current: Circuit, matcher: Matcher) -> Circuit:
        ...
        """
        "SOLUTION"
        found = matcher(current)
        if found:
            current = self.transform(current)

        new_inps = [self._update_impl(inp, matcher) for inp in current.inputs]
        any_changed = any(n is not o for (o, n) in zip(current.inputs, new_inps))
        if any_changed:
            return replace_inputs(current, new_inps)
        return current
...
def replace_array(circuit: Circuit, value: t.Tensor) -> Circuit:
    ...
    # no shape check because we use this to replace non-batch -> batch
    return Array(value, name=circuit.name)


orig_inp = test_inputs[0]
new_inp = test_inputs[1]
circuit_a = model_to_circuit(model_a, orig_inp)  # Fresh copy in case they mess up
...
def einsum_prepend_batch(ein: Circuit, batch_size: int) -> Einsum:
    ...
    ein = ein.cast_einsum()
    spec = ein.spec
    new_sizes = spec.sizes.copy()
    index = string.ascii_letters.index("b")
    assert (
        new_sizes[index] == -1
    )  # only because I used this convention previously - this value was defined to be arbitrary
    new_sizes[index] = batch_size
    return Einsum(
        *ein.inputs,
        spec=EinsumSpec(
            [spec.input_axes[0], (index,) + spec.input_axes[1]],
            (index,) + spec.output_axes,
            new_sizes,
        ),
        name=ein.name,
    )
...
batch_circuits: list[tuple[str, Circuit]] = [
    ("A", batch_circuit_a),
    ("B", batch_circuit_b),
]
...
def make_replacer(value: t.Tensor) -> TransformIn:
    ...
    "SOLUTION"
    updater = BasicUpdater(partial(replace_array, value=value))
    return partial(updater.update, matcher=Matcher("x"))
...
"""
# In rc you would use Expander here or call expand_node, but these are way more general than what we need here. I think that we shouldn't introduce expansion until we need to do symbolic sizes anyway.
...
if "SOLUTION":
    for name, batch_circuit in batch_circuits:
        zero_ablated = batch_circuit.update(Relu, lambda c: Array(t.zeros(c.shape), name=c.name))
        result = circuit_test(zero_ablated, test_dataset)
        results[("circuit_zero", name)] = result
else:
...
if "SOLUTION":
    ...
    def mean_replacer(c: Circuit) -> Array:
        b, h = c.shape
        return Array(repeat(cmean, "h -> b h", b=b), name=c.name)

    for name, batch_circuit in batch_circuits:
        normal = replacer(batch_circuit)
        the_relu = Matcher(Relu).get_first(normal)
        assert the_relu is not None
        cmean = the_relu.evaluate().mean(dim=0)
        mean_ablated = normal.update(Relu, mean_replacer)
        results[("circuit_mean", name)] = circuit_test(mean_ablated, test_dataset)
else:
...
if "SOLUTION":
    for name, batch_circuit in batch_circuits:
        normal = replacer(batch_circuit)
        first = Matcher("first").get_first(normal)
        assert first is not None
        first_acts = first.evaluate()
        shuffled = first_acts[t.randperm(len(first_acts))]
        resample_ablated = normal.update("first", lambda c: Array(shuffled, name=c.name))
        results[("circuit_resample", name)] = circuit_test(resample_ablated, test_dataset)
...
if "SOLUTION":
    ...
    def skip_transform(add: Circuit) -> Circuit:
        add = add.cast_add()
        relu, x = add.inputs
        shuffled = test_inputs[t.randperm(len(test_inputs))]
        return Add(relu, Array(shuffled, name="x_ablated"), name=add.name)

    for name, batch_circuit in batch_circuits:
        skip_ablated = batch_circuit.update("skip", skip_transform)
        results[("circuit_skip_ablated", name)] = circuit_test(skip_ablated, test_dataset)
...
class IterativeMatcher:
    ...
    def chain(self, *rest: IterativeMatcherIn) -> IterativeMatcher:
        ...
        "SOLUTION"
        # TBD: don't understand why we need the cast here - problem with subclasses
        chain = [cast(IterativeMatcher, self)] + [IterativeMatcher(r) for r in rest]
        md = IterativeMatcherDataChain([chain])
        return IterativeMatcher._from_data(md)
    ...
    def _get_internal(self, circuit: Circuit, out: set[Circuit]) -> None:
        ...
        """
        "SOLUTION"
        result = self.match_iterate(circuit)
        if result.found:
            out.add(circuit)
        for child, next_im in zip(circuit.children(), result.updated):
            if next_im is None:
                continue
            next_im._get_internal(child, out)
    ...
    def get_unique(self, circuit: Circuit) -> Circuit:
        ...
        # Note: could short circuit after finding 2 matches.
    ...
    def update(self, circuit: Circuit, transform: TransformIn) -> Circuit:
        return Updater(transform).update(circuit, self)  # TBD
...
class IterativeMatcherDataPlain(IterativeMatcherDataBase):
    ...
    def match_iterate(self, current: IterativeMatcher, circuit: Circuit) -> IterateMatchResults:
        "SOLUTION"
        return IterateMatchResults(found=self.matcher(circuit), updated=[current for _ in circuit.inputs])


# %%
print("Testing IterateMatchResults")
r = IterateMatchResults(False, [None])  # type: ignore
...
class IterativeMatcherDataFilter(IterativeMatcherDataBase):
    ...
    def match_iterate(self, current: IterativeMatcher, circuit: Circuit) -> "IterateMatchResults":
        ...
        # Easy case: past the end - terminate since we can't match
        ...
        # Easy case: haven't reached start depth yet - children increment current depth
        ...
        if isinstance(r.updated, list):
            # implementation detail - if current_depth + 1 == end_depth we can return terminate now
            updated = []  # TBD lowpri: typechecker complains if typing this list
...
def restrict(
    ...
    "SOLUTION"
    md = IterativeMatcherDataFilter(im, term_if_matches, start_depth, end_depth, Matcher(term_early_at))
    return IterativeMatcher._from_data(md)
...
"""
# %%
Chain = list["IterativeMatcher"]  # rc.ChainItem with first, rest - here just a plain list
...
def process_chain(chain: Chain, circuit: Circuit, mutable_new_items: list[list[Optional[Chain]]]) -> bool:
    ...
    """
    "SOLUTION"
    # see Rust fn match_iterate - I still feel confused on this
    first, *rest = chain
    im_result = first.match_iterate(circuit)
    if not im_result.all_finished():
        new_chains = []
        for next_im in im_result.updated:
            if next_im is not None:
                new_chain = chain.copy()
                new_chain[0] = cast("IterativeMatcher", next_im)
                new_chains.append(new_chain)
        new_item = new_chains
        mutable_new_items.append(new_item)

    if im_result.found:
        if rest:
            # TBD high: explain this wacky part - we can match immediately on rest, but this is to be configured by flag...
            return process_chain(rest, circuit, mutable_new_items)
        return True  # chain finished
    return False
...
class IterativeMatcherDataChain(IterativeMatcherDataBase):
    ...
    def match_iterate(self, current: IterativeMatcher, circuit: Circuit) -> IterateMatchResults:
        ...
        """
        "SOLUTION"
        mutable_new_items: list[list[Optional[Chain]]] = []
        any_chain_finished = False
        for chain in self.chains:
            is_chain_finished = process_chain(chain, circuit, mutable_new_items)
            any_chain_finished = any_chain_finished or is_chain_finished

        out = [[] for _ in circuit.inputs]
        for item in mutable_new_items:
            for child_chains, x in zip(out, item):
                if x is not None:
                    child_chains.append(x)
        new_chains = [chain for chain in out if out]

        if new_chains == self.chains:
            # Gnarly: this is just an optimization to avoid making new objects
            # but semantically we could just use the else block
            next_im = current
        else:
            next_im = IterativeMatcher._from_data(IterativeMatcherDataChain(new_chains))
        return IterateMatchResults(any_chain_finished, [next_im for _ in circuit.inputs])


# TBD: write full working solution for this
...
if False:
    # TBD: solution doesn't pass these tests yet - complete solution and make sure these tests are good
    ...
    # TBD: what should chains be after this? No way to inspect the Rust version sadly!
...
# TBD: test match exact same node, though this may change in real circuits:
# rc.IterativeMatcher(rc.Add).chain(rc.Add).get(rc.Add(rc.Add(name='B'), rc.Add(name='C'), name='A'))
...
class Updater:
    ...
    def _update_impl(self, current: Circuit, matcher: IterativeMatcher) -> Circuit:
        ...
        """
        "SOLUTION"
        r = matcher.match_iterate(current)
        if r.found:
            current = self.transform(current)

        new_inps: list[Circuit] = []
        for i, inp in enumerate(current.inputs):
            next_matcher = r.updated[i]
            if next_matcher is None:
                new_inp = inp
            else:
                new_inp = self._update_impl(inp, next_matcher)
...
if "SOLUTION":
    for name, batch_circuit in batch_circuits:
        im = restrict(IterativeMatcher("x"), end_depth=4)
        shuffled = test_inputs[t.randperm(len(test_inputs))]
        ablated = Updater(lambda c: Array(shuffled, name="x_ablated")).update(batch_circuit, im)
        results[("circuit_skip_ablated_2", name)] = circuit_test(ablated, test_dataset)
else:
...
def distribute_once(ein: Einsum, operand_idx: int) -> Add:
    ...
    """
    "SOLUTION"
    add = ein.inputs[operand_idx].cast_add()
    summands = []
    for inp in add.inputs:
        new_args = ein.inputs.copy()
        new_args[operand_idx] = inp
        summands.append(Einsum(*new_args, spec=ein.spec, name=""))
    return Add(*summands, name=f"{ein.name}_dist")
...
def shape_after_index(shape: tuple[int, ...], index: Sequence[TorchAxisIndex]) -> tuple[int, ...]:
    ...
    """
    "SOLUTION"
    # TBD lowpri: range validation is pretty easy - make them do it?
    out = []
    for dim_idx, s in enumerate(shape):
        if dim_idx >= len(index):
            out.append(s)
        else:
            val = index[dim_idx]
            if isinstance(val, int):
                pass  # this dimension is removed from the output
            elif isinstance(val, slice):
                out.append(len(range(s)[val]))  # Janky but works?
            else:
...
class Index(Circuit):
    ...
    def __init__(self, node: Circuit, index: Sequence[TorchAxisIndex], name: Optional[str] = None):
        ...
        self.shape = shape_after_index(node.shape, index)

    def evaluate(self) -> t.Tensor:
        "SOLUTION"
        inp = self.inputs[0].evaluate()
...
def push_down_index_once(node: Index) -> Union[Index, Relu, Add]:
    ...
    if isinstance(inp, Array):
        return node
    elif isinstance(inp, Relu):
        assert len(inp.inputs) == 1
        relu_arg = inp.inputs[0]
        return Relu(Index(relu_arg, index=node.index))
    elif isinstance(inp, Add):
        return Add(*[Index(inp, index=node.index) for inp in inp.inputs])
    else:
        raise ValueError("Wrong type for input of index: ", inp)


# TBD: fix naming scheme throughout - it's kind of sad currently
